...
contract HintHelpers is blocPayBase, Ownable, CheckContract {
        ...
        while (currentTroveuser != address(0) && remainingLUSD > 0 && _maxIterations-- > 0) {
            ...
            if (netLUSDDebt > remainingLUSD) {
                if (netLUSDDebt > MIN_NET_DEBT) {
                    uint maxRedeemableLUSD = blocPayMath._min(remainingLUSD, netLUSDDebt.sub(MIN_NET_DEBT));
                    ...
                    partialRedemptionHintNICR = blocPayMath._computeNominalCR(newColl, compositeDebt);
    ...
    */
        ...
        returns (address hintAddress, uint diff, uint latestRandomSeed)
    {
        ...
        diff = blocPayMath._getAbsoluteDifference(_CR, troveManager.getNominalICR(hintAddress));
        ...
        while (i < _numTrials) {
            ...
            uint currentDiff = blocPayMath._getAbsoluteDifference(currentNICR, _CR);
    ...
    function computeNominalCR(uint _coll, uint _debt) external pure returns (uint) {
        return blocPayMath._computeNominalCR(_coll, _debt);
    }

    function computeCR(uint _coll, uint _debt, uint _price) external pure returns (uint) {
        return blocPayMath._computeCR(_coll, _debt, _price);
    }
}
