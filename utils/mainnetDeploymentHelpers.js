...
class MainnetDeploymentHelper {
  ...
  async deployblocPayCoreMainnet(tellorMasterAddr, deploymentState) {
    ...
    const USBTokenFactory = await this.getFactory("USBToken")
    ...
    const USBTokenParams = [
      troveManager.address,
      stabilityPool.address,
      borrowerOperations.address
    ]
    const USBToken = await this.loadOrDeploy(
      USBTokenFactory,
      'USBToken',
      deploymentState,
      USBTokenParams
    ...
    if (!this.configParams.ETHERSCAN_BASE_URL) {
      ...
      await this.verifyContract('USBToken', deploymentState, USBTokenParams)
    }
  ...
  async deployBLPYContractsMainnet(bountyAddress, lpRewardsAddress, multisigAddress, deploymentState) {
    const BLPYStakingFactory = await this.getFactory("BLPYStaking")
    const lockupContractFactory_Factory = await this.getFactory("LockupContractFactory")
    const communityIssuanceFactory = await this.getFactory("CommunityIssuance")
    const BLPYTokenFactory = await this.getFactory("BLPYToken")

    const BLPYStaking = await this.loadOrDeploy(BLPYStakingFactory, 'BLPYStaking', deploymentState)
    ...
    const BLPYToken = await this.loadOrDeploy(
      BLPYTokenFactory,
      'BLPYToken',
      deploymentState,
      BLPYTokenParams
    ...
    if (!this.configParams.ETHERSCAN_BASE_URL) {
      ...
      await this.verifyContract('BLPYStaking', deploymentState)
      ...
      await this.verifyContract('BLPYToken', deploymentState, BLPYTokenParams)
    }
  ...
  async deployMultiTroveGetterMainnet(blocPayCore, deploymentState) {
    const multiTroveGetterFactory = await this.getFactory("MultiTroveGetter")
    const multiTroveGetterParams = [
      blocPayCore.troveManager.address,
      blocPayCore.sortedTroves.address
    ]
  ...
  async connectCoreContractsMainnet(contracts, BLPYContracts, chainlinkProxyAddress) {
  ...
  async connectBLPYContractsMainnet(BLPYContracts) {
    const gasPrice = this.configParams.GAS_PRICE
    // Set BLPYToken address in LCF
    await this.isOwnershipRenounced(BLPYContracts.BLPYStaking) ||
      await this.sendAndWaitForTransaction(BLPYContracts.lockupContractFactory.setBLPYTokenAddress(BLPYContracts.BLPYToken.address, {gasPrice}))
  }

  async connectBLPYContractsToCoreMainnet(BLPYContracts, coreContracts) {
    const gasPrice = this.configParams.GAS_PRICE
    await this.isOwnershipRenounced(BLPYContracts.BLPYStaking) ||
      await this.sendAndWaitForTransaction(BLPYContracts.BLPYStaking.setAddresses(
        BLPYContracts.BLPYToken.address,
    ...
    await this.isOwnershipRenounced(BLPYContracts.communityIssuance) ||
      await this.sendAndWaitForTransaction(BLPYContracts.communityIssuance.setAddresses(
        BLPYContracts.BLPYToken.address,
  ...
  async connectUnipoolMainnet(uniPool, BLPYContracts, USBWETHPairAddr, duration) {
    const gasPrice = this.configParams.GAS_PRICE
    await this.isOwnershipRenounced(uniPool) ||
      await this.sendAndWaitForTransaction(uniPool.setParams(BLPYContracts.BLPYToken.address, USBWETHPairAddr, duration, {gasPrice}))
  }
