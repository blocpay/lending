...
async function main() {
  ...
  const coreContracts = await dh.deployblocPayCoreHardhat()
  const ARBITRARY_ADDRESS = "0x5f4eC3Df9cbd43714FE2740f5E3616155c5b8419" 
  const BLPYContracts = await dh.deployBLPYContractsHardhat(
      ARBITRARY_ADDRESS, 
      ARBITRARY_ADDRESS,
      ARBITRARY_ADDRESS
    )
 ...
 const { troveManager, borrowerOperations, hintHelpers, sortedTroves, priceFeedTestnet } = coreContracts
  ...
  await dh.connectCoreContracts(coreContracts, BLPYContracts)
  await dh.connectBLPYContracts(BLPYContracts)
  await dh.connectBLPYContractsToCore(BLPYContracts, coreContracts)
  ...
  const USBAmount = toBN(toWei('2500')) // borrower wants to withdraw 2500 USB
  ...
  const liquidationReserve = await troveManager.USB_GAS_COMPENSATION()
  const expectedFee = await troveManager.getBorrowingFeeWithDecay(USBAmount)
  
  // Total debt of the new trove = USB amount drawn, plus fee, plus the liquidation reserve
  const expectedDebt = USBAmount.add(expectedFee).add(liquidationReserve)
  ...
  await borrowerOperations.openTrove(maxFee, USBAmount, upperHint, lowerHint, { value: ETHColl })

  // --- adjust trove --- 

  const collIncrease = toBN(toWei('1'))  // borrower wants to add 1 ETH
  const USBRepayment = toBN(toWei('230')) // borrower wants to repay 230 USB

  // Get trove's current debt and coll
  const {0: debt, 1: coll} = await troveManager.getEntireDebtAndColl(borrower)
  
  const newDebt = debt.sub(USBRepayment)
  ...
  await borrowerOperations.adjustTrove(maxFee, 0, USBRepayment, false, upperHint, lowerHint, {value: collIncrease})


  // --- RedeemCollateral ---

  // Get the redemptions hints from the deployed HintHelpers contract
  const redemptionhint = await hintHelpers.getRedemptionHints(USBAmount, price, 50)

  const {0: firstRedemptionHint, 1: partialRedemptionNewICR, 2: truncatedUSBAmount} = redemptionhint
  ...
  */
  ...
  /* Finally, perform the on-chain redemption, passing the truncated USB amount, the correct hints, and the expected
  * ICR of the final partially redeemed trove in the sequence. 
  */
  await troveManager.redeemCollateral(truncatedUSBAmount,
    firstRedemptionHint,
